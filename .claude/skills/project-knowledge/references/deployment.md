...
- **Production**: VPS `213.139.208.8`, app at `/var/www/yt2mp3`, run as a `systemd` service named `yt2mp3` via `gunicorn --workers 1 --threads 4 --timeout 300 --bind 127.0.0.1:5000 --no-control-socket app:app` (both `--threads` and `--timeout` come from a drop-in, not the unit file — see Request Concurrency and Timeout below), as the unprivileged `yt2mp3` system user (see Service User & Hardening below) — the single *worker* is load-bearing: all in-memory state — `jobs`, `info_cache`, `request_times` — lives in one process with no cross-process sharing, so adding workers would silently break rate limiting, the info cache, and job status polling. **Threads are a different matter and are safe** — they share that same process memory, which is exactly why concurrency was added that way rather than by raising the worker count.
...
## Request Concurrency and Timeout (gunicorn drop-in, added 2026-08-22, threads 2026-08-25)

`--threads 4` and `--timeout 300` are set via a systemd drop-in at `/etc/systemd/system/yt2mp3.service.d/timeout.conf` (filename is historical — it re-declares the whole `ExecStart` now), **not** by editing the unit file. The drop-in blanks `ExecStart` first, as systemd requires, and is reverted by deleting that one file plus a `daemon-reload`. Because it lives outside the git repo, `deploy.yml` never touches it; it survives deploys — verified — but would be lost in a server rebuild, so it has to be recreated by hand alongside the unit itself.

**Why threads.** Without them the single sync worker serves exactly one request at a time, so any slow request stalls the entire site for everyone: a large `/api/file` download on a mobile connection, or an `/api/info` lookup while yt-dlp waits on a slow source. Threads fix that without touching the single-process constraint above. The app was already multi-threaded before this — `run_download` runs in a background thread and writes to `jobs` while requests read it — so request threads introduce no new class of problem. `info_cache`, `request_times` and `thumb_request_times` are each already guarded by their own lock; `jobs` is not, but only ever sees whole-value `get`/`set`/`pop`, which are atomic under the GIL. The one genuinely racy spot is `/api/file`, where two concurrent requests for the same job could both pass the readiness check — harmless in practice, since deleting an open file on Linux leaves the in-flight transfer's descriptor valid and `_cleanup_file` already uses `unlink(missing_ok=True)`.

Four threads on a one-core, 2GB box is deliberate rather than a formula: the work these threads do is I/O-bound (blocked on sockets, GIL released), and the CPU-heavy part — ffmpeg — runs in a separate process entirely.
