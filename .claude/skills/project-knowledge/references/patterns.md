...
Silencing the progress bar still left yt-dlp's *error* text arriving mangled — the lines that matter most — because it carries ANSI colour codes and a leading `\r`, both of which defeat journald the same way. Two pieces address that, and new code should use them rather than reaching for `logger.exception`: every `YoutubeDL` is constructed with `"logger": _ytdlp_logger`, so yt-dlp reports through the app logger instead of writing to stdout behind its back, and failures are recorded with `log_exception()`, which scrubs the *whole formatted traceback* — scrubbing just the message is not enough, since the traceback's final line repeats it verbatim.
