...
2. Background thread: yt-dlp downloads best audio stream (progress hook updates `jobs[job_id]` 0→90%), then `ffmpeg` re-encodes/remuxes to the target format (progress parsed from ffmpeg's own `-progress` output, mapped 90→100%). `run_download` catches failures in two tiers (see `UserFacingError` below): a deliberate business rejection surfaces its own Russian message with no traceback, anything else calls `log_exception(e)` and shows one generic Russian message — a background thread has no HTTP response to surface a traceback through, so without the logging call the only trace of a download-phase failure was the client-side `job_error` Umami event, and server logs stayed silent. This was a real gap, not theoretical: a yt-dlp VK-extractor bug (`extractor/vk.py`, a subtitle entry with `title: null` crashing `.split()`) took down two real users' downloads with zero corresponding `journalctl` entry until the logging call was added — see `patterns.md` → Background Thread Error Handling for the general rule this establishes.

   **`UserFacingError`** (defined just above `run_download`) marks an exception as a deliberate, already-Russian, ready-to-show message — raised only from a plain condition check, never wrapping another exception (so a real bug can't accidentally get misclassified as an expected rejection and lose its traceback). Two cases use it: rejecting a playlist link, and the corrupt/near-empty trim-output guard. `run_download` catches `UserFacingError` first (message shown as-is, no logging — these are routine, not bugs) and falls through to a broader `except Exception` for everything else (yt-dlp/ffmpeg internals, a missing downloaded file, an ffmpeg failure) — logged in full, shown to the user as one generic message ("Не удалось обработать это видео. Попробуйте другое или другую ссылку.") rather than a raw English exception string. The playlist-rejection message is identical, verbatim, to the one `get_info` returns for the same condition (`/api/info`'s own pre-check, a plain early `return` rather than a raised exception) — kept in sync deliberately, so preview and download don't disagree about the same input.
...
`run_download`'s `outtmpl` caps the title at 80 characters (`%(title).80s`). The constraint that makes this necessary is easy to miss: `NAME_MAX` is 255 **bytes**, not characters, and Cyrillic costs two bytes each — so a title of about 130 characters already overflows, while an English one of the same length is fine.

yt-dlp does trim the base name to fit by itself, which is why this looked safe. It isn't, because fragmented downloads then append `.part-Frag1000.part` on top of the already-trimmed name. Two jobs on 2026-09-05 failed this way and retried 96 times each, accounting for 192 of that week's 218 errors; the arithmetic came out one byte over the limit. The cap leaves roughly 60 bytes of headroom for the job id, the extension and that suffix.

Only the on-disk name is shortened. What the user downloads is named from `display_title`, taken from the extracted metadata rather than from the file, so the full title survives. `filename_too_long` is kept as a classified cause anyway — the cap counts characters while the limit counts bytes, so 80 four-byte characters could still overflow, and if that happens it should read as ours rather than as a bad link.

## Livestreams Are Rejected

A livestream download never finishes, so it is refused before a single byte is written. Both `/api/info` and `run_download` check `is_live` on the same cheap `extract_flat` probe the playlist check already uses — verified that the flat probe does report it (`is_live: True`, `duration: None`), so no extra network call is needed. The message is one shared constant, since both endpoints show it and `patterns.md` requires identical wording for a condition visible from more than one place.

Added 2026-08-25 after a production incident: three concurrent jobs on a 24/7 music stream filled `downloads/` with 6.7GB and took the disk to 86% with roughly fifteen minutes of headroom left, while the worker sat at 852MB resident with three threads stuck over an hour. Nothing self-corrects during that — the janitor sweeps by `mtime`, which an actively-written file keeps refreshing, so a running download is permanently "fresh" and never swept. This also explains the unattributed ~850MB-1GB memory peaks seen since 2026-08-19; there was no leak.

Distinct from the VK `/live-` URL check above, which recognises a livestream from the URL shape alone because yt-dlp's VK extractor refuses to claim those links at all.

## Download Size Cap

`MAX_DOWNLOAD_BYTES` (1.5 GiB) aborts a job that keeps growing, as a backstop for anything the `is_live` check doesn't catch — a source that misreports itself, or a merely enormous VOD. It is enforced by hand in `progress_hook`, which raises `UserFacingError`; that propagates out of `extract_info` unwrapped, so it lands in the dedicated except clause rather than the generic one.

yt-dlp's own `max_filesize` is **not** used, because it wouldn't have helped: it only compares a *declared* `Content-Length` before starting, and the fragmented HLS/DASH downloaders — the ones a livestream actually uses — never consult it. The ceiling sits well above any plausible audio-only download; multi-GB pulls are reachable at all only because `bestaudio/best` can fall back to a full video stream.
...
Every event carries `source`, never the full URL or video ID (see `getSourceLabel(url)`): the URL's hostname, normalized to `youtube` / `tiktok` / `soundcloud` / `vk` / `other`. Both error events additionally carry `reason` — the stable code from `classify_extraction_error`/`classify_known_bad_link`/`UserFacingError` (see Generic Extraction-Failure Classification below), which is what turns "something broke on VK" into "18 of these were livestreams". Only the code is sent, never the Russian message text and never the URL. Two codes are the ones worth watching: `extractor_outdated` means a source changed its markup and yt-dlp needs bumping, and a rising `unclassified` means a failure nobody has named yet. Matching is by `endsWith`-subdomain, not an exact-host list — `music.youtube.com` and `m.youtube.com` both land under `youtube` rather than `other`, which matters in practice since mobile share-target links (see PWA below) commonly arrive as `m.youtube.com`. Both of VK's live video domains are covered (`vk.com` — what copy-pasting a link from the VK web UI produces — and `vkvideo.ru`, the site's own `/vk` placeholder domain) and normalize to the same `vk` label. An unparseable/empty URL (`new URL()` throws) → `other`, never a crash.
...
**Conditional load by hostname.** The Umami `<script>` tag (and its `preconnect` hint) is injected via an inline script guarded by `location.hostname === 'audiograb.ru' || location.hostname === 'www.audiograb.ru'`, on all five pages that load it (`index.html`, `tiktok.html`, `soundcloud.html`, `vk.html`, `thumbnail.html`) — not a static tag. Before this, opening any page via `file://`, a local Flask dev server, or the browser-preview tool still loaded the real production Umami and reported `location.pathname`, which for `file://` is the full local filesystem path — local/test runs were leaking into the production dashboard as pageviews, and since `trackEvent` only checks whether `window.umami` exists (not the environment), the same was true for `job_done`/`job_error`/`preview_error`/`share_used` — polluting the exact per-source error-rate signal those events exist to produce. `trackEvent` itself is unchanged: it already no-ops silently when `window.umami` is absent, which is now simply the case everywhere except production.

**Both `audiograb.ru` and `www.audiograb.ru` are matched.** When the guard was written, nginx served both hostnames directly and matching only the bare domain would have created a real analytics blind spot for anyone landing on `www.`. That is no longer the case — `www` has redirected to the bare domain with a 301 since 2026-07-27 (see `deployment.md` → Domain Canonicalization), so in practice no page ever loads under `www` any more. The second hostname is kept in the guard anyway: it costs nothing, and it means an nginx change can't silently switch analytics off.
...
Safe as a single in-process thread only because production runs gunicorn with exactly one worker *process* (see `deployment.md`) — with multiple workers each would run its own janitor over its own process-local state, same as the rate-limiting/cache/jobs correctness this whole app depends on. The `--threads` setting is unrelated: threads share one process, so there is still exactly one janitor.

## Failed Jobs Clear Their Own Files

`run_download`'s `finally` block deletes everything matching the job id when the job ended in `status == "error"`, guarded on that status so a successful job keeps the output `/api/file` still has to serve.

Without it, every failure parks its partial file on disk for up to an hour until the janitor's age sweep notices — and the janitor is the only thing that would. That is not a rare path: read timeouts and source 5xx happen regularly (two dozen in a single three-day window), and the size cap above would otherwise leave behind exactly the 1.5 GiB it just refused to finish downloading.
...
[classify_known_bad_link()](app.py) catches five specific bad-input shapes before yt-dlp ever runs, positioned right after `validate_url` in both `/api/info` and `/api/download` — same synchronous, no-network-call placement, same rationale (don't burn a rate-limit slot or a yt-dlp call on input that can never succeed). It returns the Russian message paired with the Umami reason code for that shape.
...
- **Two links pasted into one another** — checked first, since a splice wrecks the path parsing everything below depends on. Detected by counting `://` in the part before the query string, so a URL legitimately carrying another in a redirect parameter isn't caught. This was our own doing: the URL field kept its value after a finished job, so the next paste landed at the caret — mid-string on a phone — splicing a whole URL into the middle of the old one. The field is cleared on completion now (see Blob-Once Download/Share Pattern), but the check stays, because the same splice is easy to make in any text and the generic "unsupported link" wording hid what was actually wrong. Six occurrences on 2026-08-27..28, including one user retrying 16 seconds later and hitting it again.
- **Yandex.Video wrapper links** (`/video/preview/...`) — Yandex indexes and embeds VK videos in its own search results; copying a link from there hands the user a Yandex share URL instead of the actual VK one. Host and path both mirror yt-dlp's `YandexVideoPreviewIE` rather than the shapes happened to be seen first: any yandex TLD (`.com`, `.kz`, `.com.tr`…) plus `ya.ru`, and both the `/video/preview/<id>` path and the `/video/preview?filmId=<id>` query form. It was narrower until 2026-08-28 — `ya.ru`/`yandex.ru` with a required trailing slash — and the query form slipped through to yt-dlp, where it surfaced as `Unable to extract data_raw`, i.e. as Yandex having changed its markup rather than as this check being too tight.
...
- **VK podcasts** (`/podcast-<owner>_<id>`) — added 2026-08-27. Not a temporary gap: yt-dlp has no VK *audio* support at all — the word doesn't appear in its `vk.py`, whose `_VALID_URL` claims only `video`/`clip`/embed forms, and every podcast URL shape was confirmed to fall through to the generic extractor. VK guards audio far more tightly than video (it has supported VK video for years without ever adding audio), so building this ourselves would mean a logged-in account's token on the server, ban risk, and permanent maintenance of the app's most fragile piece — deliberately not attempted. The message therefore points at the video version of an episode, which does work, instead of inviting a retry. Matched on the `/podcast-` prefix, so the `/podcasts` listing page is not caught.
- **VK livestreams** (`/live-<owner>_<id>` on the same VK hosts) — added 2026-08-22 after a third occurrence in the server log. yt-dlp's VK extractor doesn't claim these at all, so they fall through to the generic extractor and surface as a bare `UnsupportedError`, whose "check that this is a direct link" wording is misleading: the link is perfectly valid, the format simply isn't supported. Distinct from the `is_live` rejection below, which catches livestreams the extractor *does* claim, by asking the source rather than reading the URL.
...
[classify_extraction_error(e, fallback)](app.py) sits one layer below `classify_known_bad_link`: it runs *after* yt-dlp has already failed (in both `get_info`'s and `run_download`'s `except` blocks), and turns yt-dlp's own exception text into a more specific Russian message where the failure has a known, user-relevant cause. It returns `(message, reason_code)` — the code is what reaches Umami, and it must stay stable when the Russian wording is edited, since the dashboard groups on it. Anything that doesn't match falls through to the caller-supplied generic fallback (paired with `unclassified`), which differs between the two call sites: pure metadata lookup vs. also covering ffmpeg/filesystem failures. Added 2026-08-16 alongside the `yandex.ru` fix above.

Recognised causes, in match order: unsupported site, unparseable URL, login requirement, age restriction, geo restriction, a bad TLS certificate at the source, a transient source failure (5xx or a read timeout), a site markup change yt-dlp hasn't caught up to, a DNS failure on our own server, a source returning no downloadable formats at all, an output filename too long for the filesystem, and the extractor itself crashing. Order matters in one place: the transient check sits ahead of the markup-change check, because a timeout can surface wrapped in "Unable to extract" wording and "try again in a minute" is the useful advice there. `HTTP Error 403` is deliberately left unclassified — it means the source refused us, so telling the user to wait would be wrong.

Two of these came from reading the logs rather than from imagination, and both are worth knowing about. **`login_required` matches two wordings** because VK says "only available to signed-in users" where the rule was originally written against "registered users" — six occurrences in five days took the generic fallback despite an accurate message already existing for exactly that case. **`extractor_crashed` catches a Python exception escaping from yt-dlp** instead of a reported failure: Odnoklassniki's extractor passing an already-parsed dict into a JSON parser, and `vk.py` calling `.split()` on a null subtitle title. Neither message names its source, so the rule stays generic rather than pretending to identify one — any extractor breaking this way means the same thing to the user, and the code makes the frequency visible on the dashboard. Its message says outright that the fault is ours and the link is fine, since otherwise people retry a link that was never the problem.

**On Odnoklassniki specifically, the decision was to wait for upstream** (2026-09-04). Support is not missing — `odnoklassniki.py` exists and is claimed; it broke because OK.ru now returns `flashvars['metadata']` as an object where the extractor still expects a JSON string. Bumping yt-dlp does not help: `2026.08.19` was already the newest release available when this was diagnosed. The alternative, carrying a local patch or an extractor plugin, means taking a third-party extractor into our own maintenance for what was three log entries from one person in five days — so it is deliberately not being done. If it matters more later, the upstream fix is a type check before the parse, small enough to be worth contributing rather than vendoring.

Codes and Russian text live in one table (`_EXTRACTION_ERRORS`) rather than a separate code-matching function, which would duplicate every substring and drift away from the wording it describes. The app's own rejections — playlists, livestreams, the size cap, a corrupt trim — carry their codes on `UserFacingError` instead.

Matches on yt-dlp's own message substrings rather than exception subclasses (`DownloadError`, what actually reaches these blocks, re-wraps the original message as a plain string and doesn't reliably preserve the original exception type). This is a display-text choice only, made after the failure is already in the generic-crash branch and already logged via `log_exception` — it doesn't affect *whether* something is logged, so it doesn't conflict with `patterns.md`'s warning against using message-text matching to decide expected-vs-crash (see Background Thread Error Handling).
