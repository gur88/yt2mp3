...
2. Background thread: yt-dlp downloads best audio stream (progress hook updates `jobs[job_id]` 0→90%), then `ffmpeg` re-encodes/remuxes to the target format (progress parsed from ffmpeg's own `-progress` output, mapped 90→100%). Any exception in this thread (`run_download`'s `except Exception as e:`) calls `logger.exception(e)` before recording the failure in `jobs[job_id]`, mirroring `get_info`'s error handling — a background thread has no HTTP response to surface a traceback through, so without this the only trace of a download-phase failure was the client-side `job_error` Umami event, and server logs stayed silent. This was a real gap, not theoretical: a yt-dlp VK-extractor bug (`extractor/vk.py`, a subtitle entry with `title: null` crashing `.split()`) took down two real users' downloads with zero corresponding `journalctl` entry until this was added — see `patterns.md` → Background Thread Error Handling for the general rule this establishes.
