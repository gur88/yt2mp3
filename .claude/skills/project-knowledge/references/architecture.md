...
**Both `audiograb.ru` and `www.audiograb.ru` are matched, confirmed rather than assumed**: `curl` against `https://www.audiograb.ru/` returns a plain `200 OK` with the site's own content, not a redirect to the bare domain — nginx serves both hostnames directly with no canonicalizing 301. (Matching only the bare domain would have created a real analytics blind spot for anyone who lands on `www.`, not just filtered test noise.) This is a separate, pre-existing SEO/canonicalization gap — every page's `rel=canonical` already points at the bare domain, but nginx itself doesn't redirect `www` → bare — noted here as a possible future fix, out of scope for this change.
